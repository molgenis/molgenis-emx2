...
class Transform:
    ...
    def variable_values(self):
        ...
        df_var_values_cdm = df_var_values[df_var_values['resource'].isin(['LifeCycle', 'ATHLETE', 'testNetwork1', 'EXPANSE'])]
        df_var_values_cdm.loc[:, 'variable.name'] = df_var_values_cdm['variable.name'].apply(remove_number)

        df_var_values_no_cdm = df_var_values[~df_var_values['resource'].isin(['LifeCycle', 'ATHLETE', 'testNetwork1', 'EXPANSE'])]

        df_all_var_values = pd.concat([df_var_values_no_cdm, df_var_values_cdm])
        df_all_var_values = df_all_var_values.drop_duplicates(subset=['resource', 'variable.dataset',
                                                                      'variable.name', 'value'])
