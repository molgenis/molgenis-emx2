...
from zip_handling import Zip  # noqa: E402
...
class UpdateMeta:
    ...
    def __init__(self, database_name, database_type, update_session: UpdateClient):
        ...
        self.model_file = f"{self.path}molgenis.csv"
    ...
    def remove_files(self):
        ...
        for file in glob.glob(f"{self.path}/*.csv"):
            if not file.endswith(self.model_file):
                os.remove(file)
    ...
    def add_validations(self):
        ...
        self.remove_files()
        df_model = pd.read_csv(self.model_file, sep=",")
        df_model["validation"] = (
            df_model.apply(lambda row:
                           self.add_expression(row.tableName, row.columnName), axis=1))
        df_model = self.session.float_to_int(df_model)
        df_model.to_csv(f"{self.path}molgenis.csv", index=False)
