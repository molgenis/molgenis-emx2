...
export default {
  ...
  methods: {
    loadSchema() {
      ...
      request(
        "graphql",
        "{_schema{name,tables{name,tableType,externalSchema,labels{locale,value},descriptions{locale,value},columns{name,labels{locale,value}columnType,key,refTable,required,descriptions{locale,value}}}}}"
      )
        .then((data) => {
