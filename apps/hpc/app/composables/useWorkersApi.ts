...
import { type NormalizedJob, normalizeJob } from "./useJobsApi";
...
export async function fetchWorkers(): Promise<WorkerSummary[]> {
  ...
  try {
    ...
    const caps = (data.HpcWorkerCapabilities ||
      []) as WorkerCapabilityGraphqlRow[];
    const normalizedJobs = ((data.HpcJobs || []) as unknown[]).map(
      normalizeJob
    );
    ...
    return workers.map((w) => ({
      ...
      capabilities: caps
        .filter(
          (c) =>
            (c.worker_id && typeof c.worker_id === "object"
...
export async function issueWorkerCredential(
  ...
  return await $fetch<IssueCredentialResponse>(
    `${REST_BASE}/workers/${workerId}/credentials/issue`,
    {
      method: "POST",
      headers: hpcHeaders(),
      body: payload,
...
export async function rotateWorkerCredential(
  ...
  return await $fetch<IssueCredentialResponse>(
    `${REST_BASE}/workers/${workerId}/credentials/rotate`,
    {
      method: "POST",
      headers: hpcHeaders(),
      body: payload,
