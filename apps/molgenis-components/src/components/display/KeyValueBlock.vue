<template>
  <grid-block :heading="computedHeading">
    <read-more v-if="items.length === 1 && items[0].value" :text="items[0].value" :length="750" />
    <div v-else-if="items.length > 1">
      <div v-for="(item, index) in items" :key="index">
        <strong>{{ item.label }}</strong>
        <template v-if="Array.isArray(item.value)">
          <ul class="list-unstyled">
            <li v-for="(valueItem, valueItemIndex) in item.value" :key="valueItemIndex">
              {{ valueItem }}
            </li>
          </ul>
        </template>
...
export default {
  ...
  computed: {
    computedHeading() {
      return this.heading ? this.heading : this.items.length === 1 ? this.items[0].label : null;
    },
  },
};
</script>
