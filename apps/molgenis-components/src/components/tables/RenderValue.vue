<template>
  <span v-if="row[col.id] && Array.isArray(row[col.id])">
    <span v-for="(val, idx) in row[col.id]" :key="idx">
      <HyperlinkDisplay v-if="col.columnType === 'HYPERLINK_ARRAY'" :data="val" />
  ...
  <span v-else-if="row[col.id]">
    <HyperlinkDisplay v-if="col.columnType === 'HYPERLINK'" :data="row[col.id]" />
...
export default {
  ...
  methods: {
    getValue(col, row) {
      return this.getLabel(col) ? this.applyJsTemplate(getLabel(col), row[col.id]) : row[col.id];
    },
    getLabel(col) {
