<template>
    ...
    <div v-else>
        <IconBar><label>{{count}} schemas found</label>
            <IconAction icon="plus" @click="openCreateSchema"/>
        </IconBar>
        <DataTable :columns="['name', 'description']" :rows="schemas" @click="openGroup">
            <template v-slot:rowheader="slotProps">
                <IconDanger icon="trash" @click="openDeleteSchema(slotProps.row.name)" :key="slotProps.row.name"/>
            </template>
        </DataTable>
        <SchemaCreateModal v-if="showCreateSchema" @close="closeCreateSchema"/>
        <SchemaDeleteModal v-if="showDeleteSchema" @close="closeDeleteSchema" :schemaName="showDeleteSchema"/>
    </div>
</template>
...
<script>
    ...
    import {Spinner, DataTable, IconBar, IconAction, IconDanger} from "@mswertz/emx2-styleguide";
    ...
    export default {
        ...
        methods: {
            openGroup(group) {
                window.open("/apps/schema/#/" + group.name, "_blank");
            },
            openCreateSchema() {
