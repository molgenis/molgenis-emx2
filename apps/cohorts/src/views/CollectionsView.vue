...
export default {
  ...
  methods: {
    reload() {
      ...
      request(
        "graphql",
        `query Collections($filter:CollectionsFilter,$offset:Int,$limit:Int){Collections(offset:$offset,limit:$limit,${searchString}filter:$filter){name,tables{name,variables{name}}}
