...
test("Refreshing and going back should correctly set the filters in the url", async ({
  ...
  //filter
  await expect(page.getByText("Biobanks: 3")).toBeVisible();
  await page.getByText("Material type").click();
  await page.getByRole("checkbox", { name: "DNA", exact: true }).check();
  await expect(page.getByText("Biobanks: 1")).toBeVisible();
  await page.reload();
  await expect(page.getByText("Biobanks: 1")).toBeVisible();
  ...
  await page
    ...
    .click();
  await expect(page.getByText("Biobanks: 2")).toBeVisible();
  await page.reload();
  await expect(page.getByText("Biobanks: 2")).toBeVisible();
});
