...
export const useCheckoutStore = defineStore('checkoutStore', () => {
    ...
    function addCollectionsToSelection ({ collections, bookmark }) {
        ...
        selectedCollections.value = selectedCollections.value.concat(newCollections)
    ...
    function removeCollectionsFromSelection ({ collections, bookmark }) {
        ...
        selectedCollections.value = selectedCollections.value.filter(sc => !collectionsToRemove.includes(sc.value))

        /** this means we are back are the home screen. */
        if (bookmark) { // TODO: figure out how this is impacted on other screens besides the biobankcard view.
            checkoutValid.value = true
            // createBookmark(state.filters, state.selectedCollections)
        }
    }

    return { selectedCollections, addCollectionsToSelection, removeCollectionsFromSelection }
})
