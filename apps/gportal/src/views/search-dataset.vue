  ...
  <Page>
    <PageSection width="large" :horizontalPadding="0">
      <MessageBox v-if="noRemsHostFound" type="error">
        <p>Unable to retrieve local REMS URL. Make sure your REMS instance is configured with MOLGENIS. To check the status:</p>
        <ol>
          <li>navigate to your schema and then settings page</li>
          <li>Click the "Advanced Settings" tab</li>
          <li>In the settings table, confirm that there is a setting for <code>REMS_URL</code></li>
        </ol>
        <p>If the setting <code>REMS_URL</code> is missing, you can add it manually by clicking on the plus icon. The "key" should be <code>REMS_URL</code> and the "setting value" should be your REMS host.</p>
      </MessageBox>
      <div v-else>
        <RoutedTableExplorer
          tableName="Dataset"
...
import { RoutedTableExplorer, ButtonAlt, ButtonOutline } from "molgenis-components";
...
async function setRemsHost () {
  const query = gql`{ _settings { key value }}`
