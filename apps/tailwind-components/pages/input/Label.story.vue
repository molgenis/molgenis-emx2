...
<template>
  ...
  <p>
    The <code>InputLabel</code>component is a wrapper around the
    <code>label</code> element. There are a few useful options when used in
    forms and for form validation. These are displayed below. There is also a
    <code>hideLabel</code> option that visually hides the label. Every input
    should also have a label, but sometimes you may wish to hide it. In this
    case, the label is still rendered in the document and accessible, but
    visually hidden using css classes using the <code>sr-only</code> class.
    Label would appear below.
  </p>
  <div
