...
import type { ISchemaMetaData, ITableMetaData } from '../../metadata-utils/src/types';

const sampleType = ref("complex");

 // just assuming that the table is there for the demo
const schemaId = computed(() => sampleType.value === "simple" ? "pet store" : "catalogue");
const tableId = computed(() => sampleType.value === "simple" ? "Pet" : "Collections");
...
const tableMeta = computed(() => ((schemaMeta.value as ISchemaMetaData).tables.find((table) => table.id === tableId.value)) as ITableMetaData);
