...
function headingTooltip(heading: string): string | undefined {
  const headingCol = props.columns.find(
    (c) =>
      (c.columnType === "HEADING" || c.columnType === "SECTION") &&
      (c.label === heading || c.id === heading)
  );
  if (!headingCol?.description) return undefined;
  return headingCol.description;
}
...
<template>
  <div class="relative">
    <Button type="text" size="tiny" icon="plus" @click="open = !open"
      >Add filter</Button
    >
    <div
      v-if="open"
      ref="dropdown"
      class="absolute right-0 top-full mt-2 z-50 bg-modal border border-black/10 rounded-lg shadow-lg w-96"
