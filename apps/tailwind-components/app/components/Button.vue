...
const COLOR_MAPPING: Record<ButtonType, string> = {
  ...
  text: "group pl-0 pr-0 flex items-center text-button-text hover:bg-hover hover:text-link-hover cursor-pointer disabled:cursor-not-allowed disabled:text-button-text-disabled disabled:hover:bg-transparent border-none h-auto",
  outline:
  ...
  inline:
    "tracking-widest bg-none text-button-inline border-none hover:text-button-secondary rounded-full hover:bg-button-inline-hover disabled:text-button-inline-disabled disabled:border-0 disabled:hover:bg-transparent disabled:hover:text-button-inline-disabled",
};
...
// text and inline define their own disabled colors in COLOR_MAPPING;
// the disabled: variant is needed to win over the base and hover: color classes
const disabledClasses = computed(() => {
  if (!props.disabled) return "";
  return props.type === "text" || props.type === "inline"
    ? "cursor-not-allowed"
    : "cursor-not-allowed disabled:bg-button-disabled disabled:text-button-disabled disabled:border-button-disabled";
});
</script>
...
<template>
  ...
  >
    ...
    <span
      :class="`${type === 'text' && !disabled ? TEXT_STYLING : ''} ${
        iconOnly ? 'sr-only' : ''
      }`"
    >
      {{ label }}
