...
export function scrollToElementInside(
  containerId: string,
  ...
  const elementOffsetWithinContainer =
    elementInsideOffsetTop - containerOffsetTop;

  const elementHeight = elementInside.offsetHeight;
  const containerHeight = scrollableContainerElement.offsetHeight;

  const topIsBeyondThreshold =
    elementHeight < containerHeight &&
    elementOffsetWithinContainer > containerHeight / 2;
