...
export function createCountFetcher(config: {
  ...
  async function fetchRefCounts(
    ...
    const currentCrossFilter = config.getCrossFilter();
    ...
    try {
      const result = await fetchGraphql(config.schemaId, query, {
        filter: { ...currentCrossFilter, ...filterValue },
      });
  ...
  async function fetchRefBaseCounts(
    options: Map<string, Record<string, unknown>>
  ): Promise<Map<string, number>> {
    if (config.columnPath.includes(".")) return new Map();
    if (options.size === 0) return new Map();
    ...
    try {
      ...
      console.warn(
        `Failed to fetch ref base counts for ${config.columnPath}:`,
        error
      );
      return new Map();
    }
  ...
  async function fetchOntologyLeafCounts(
    names: string[]
  ): Promise<Map<string, number>> {
    if (config.columnPath.includes(".")) return new Map();
  ...
  async function fetchOntologyLeafBaseCounts(
    names: string[]
  ): Promise<Map<string, number>> {
    if (config.columnPath.includes(".")) return new Map();
    if (names.length === 0) return new Map();
    ...
    try {
      ...
      console.warn(
        `Failed to fetch ontology leaf base counts for ${config.columnPath}:`,
        error
      );
      return new Map();
    }
  ...
  async function fetchOntologyParentCounts(
    names: string[]
  ): Promise<Map<string, number>> {
    if (config.columnPath.includes(".")) return new Map();
    if (names.length === 0) return new Map();
    ...
    for (const { alias, name } of aliases) {
      ...
      variables[`filter_${alias}`] = { ...currentCrossFilter, ...nestedFilter };
    }
  ...
  async function fetchOntologyParentBaseCounts(
    names: string[]
  ): Promise<Map<string, number>> {
    if (config.columnPath.includes(".")) return new Map();
    if (names.length === 0) return new Map();
    ...
    for (const { alias, name } of aliases) {
      ...
      variables[`filter_${alias}`] = nestedFilter;
    }
    ...
    try {
      ...
      console.warn(
        `Failed to fetch ontology parent base counts for ${config.columnPath}:`,
        error
      );
      return new Map();
    }
  ...
  return {
    ...
    fetchRefCounts,
    fetchRefBaseCounts,
    fetchOntologyLeafCounts,
    fetchOntologyLeafBaseCounts,
    fetchOntologyParentCounts,
    fetchOntologyParentBaseCounts,
  };
}
