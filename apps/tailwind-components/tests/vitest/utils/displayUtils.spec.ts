...
describe("classifyCardColumns", () => {
  it("extracts DESCRIPTION role column", () => {
    const columns = [
      col({ id: "name", columnType: "STRING", role: "TITLE" }),
      col({ id: "bio", columnType: "TEXT", role: "DESCRIPTION" }),
      col({ id: "status", columnType: "STRING", role: "DETAIL" }),
    ...
    expect(result.descriptionColumn?.id).toBe("bio");
  });

  it("returns undefined descriptionColumn when no DESCRIPTION role", () => {
    const columns = [col({ id: "name", columnType: "STRING", role: "TITLE" })];
    const result = classifyCardColumns(columns);
    expect(result.descriptionColumn).toBeUndefined();
  });

  it("extracts DETAIL role columns", () => {
    const columns = [
      col({ id: "name", columnType: "STRING", role: "TITLE" }),
      col({ id: "type", columnType: "STRING", role: "DETAIL" }),
      col({ id: "country", columnType: "STRING", role: "DETAIL" }),
    ];
    const result = classifyCardColumns(columns);
    expect(result.detailColumns.map((c) => c.id)).toEqual(["type", "country"]);
  });

  it("falls back to non-key non-role columns when no DETAIL roles", () => {
    const columns = [
  ...
  it("fallback caps at 5 columns", () => {
    const columns = Array.from({ length: 8 }, (_, i) =>
      col({ id: `field${i}`, columnType: "STRING" })
    );
    const result = classifyCardColumns(columns);
    expect(result.detailColumns).toHaveLength(5);
  });

  it("fallback excludes key columns", () => {
    const columns = [
      col({ id: "id", columnType: "STRING", key: 1 }),
      col({ id: "status", columnType: "STRING" }),
    ];
    const result = classifyCardColumns(columns);
    expect(result.detailColumns.map((c) => c.id)).toEqual(["status"]);
  });

  it("fallback excludes HEADING and SECTION column types", () => {
    const columns = [
      col({ id: "sec", columnType: "SECTION" }),
      col({ id: "head", columnType: "HEADING" }),
      col({ id: "status", columnType: "STRING" }),
    ];
    const result = classifyCardColumns(columns);
    expect(result.detailColumns.map((c) => c.id)).toEqual(["status"]);
  ...
  it("fallback excludes mg_ prefixed columns", () => {
    const columns = [
      col({ id: "mg_insertedOn", columnType: "DATETIME" }),
      col({ id: "status", columnType: "STRING" }),
    ];
  ...
  it("extracts LOGO role column", () => {
    const columns = [
      col({ id: "name", columnType: "STRING", role: "TITLE" }),
      col({ id: "logo", columnType: "FILE", role: "LOGO" }),
    ];
    const result = classifyCardColumns(columns);
    expect(result.logoColumn?.id).toBe("logo");
  });

  it("returns undefined logoColumn when no LOGO role", () => {
    const columns = [col({ id: "name", columnType: "STRING" })];
    const result = classifyCardColumns(columns);
    expect(result.logoColumn).toBeUndefined();
  });

  it("returns empty detailColumns for empty input", () => {
    const result = classifyCardColumns([]);
    expect(result.detailColumns).toEqual([]);
    expect(result.descriptionColumn).toBeUndefined();
    expect(result.logoColumn).toBeUndefined();
  });

  it("prefers explicit DETAIL over fallback when DETAIL roles present", () => {
    const columns = [
      col({ id: "summary", columnType: "TEXT", role: "DETAIL" }),
      col({ id: "unroled", columnType: "STRING" }),
    ];
    const result = classifyCardColumns(columns);
