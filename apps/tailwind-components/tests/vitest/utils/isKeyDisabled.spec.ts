...
import { isKeyDisabled } from "../../../app/utils/isKeyDisabled";
...
describe("isKeyDisabled", () => {
  it("should return true if column is readonly", () => {
    ...
    expect(isKeyDisabled(rowKey, column)).toBe(true);
  });
  ...
  it("should return false if column is not readonly, not AUTO_ID and not a primary key", () => {
    const rowKey = { rowId: "1" };
    const column = { id: "id", readonly: "false", key: 0, columnType: "TEXT" };
    expect(isKeyDisabled(rowKey, column)).toBe(false);
  });

  it("should return false if column is the primary key of another column", () => {
    const rowKey = { rowId: "0" };
    const column = { id: "id", readonly: "false", key: 1, columnType: "TEXT" };
    expect(isKeyDisabled(rowKey, column)).toBe(false);
  });
});
