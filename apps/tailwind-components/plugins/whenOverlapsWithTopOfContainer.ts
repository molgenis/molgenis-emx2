...
export default defineNuxtPlugin((nuxtApp) => {
    nuxtApp.vueApp.directive('when-overlaps-with-top-of-container', {
        mounted(el: HTMLElement, binding: DirectiveBinding<LazyLoadBindingValue>) {
            ...
            if (!container) {
                console.warn('No scrollable container found for when-overlaps-with-top-of-container directive.');
                return;
            }
            ...
            const handleOverlapping = () => {;
                ...
                if (elRect.top <= containerRect.top + dynamicThreshold + 1  && elRect.bottom > containerRect.top ) {
                    binding.value();
                }
            };
