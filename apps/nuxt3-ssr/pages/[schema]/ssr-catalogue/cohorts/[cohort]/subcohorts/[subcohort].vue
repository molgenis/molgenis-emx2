...
const { data: subcohortData } = await useFetch(
    `/${route.params.schema}/catalogue/graphql`,
    {
        ...
        method: "POST",
        body: { query: queryValue, variables: { pid: route.params.cohort, name: route.params.subcohort } },
    }
...
// @ts-ignore
pageCrumbs[route.params.cohort] = `/${route.params.schema}/ssr-catalogue/cohorts/${route.params.cohort}`

function renderList(list: any[], itemMapper: (a: any) => string, itemJoiner?: (a: any) => string) {
    if (list?.length === 1) {
