...
<template>
  <nav class="flex items-center justify-between gap-6 xl:justify-center">
    <a v-for="button in mainButtons" :href="button.link"
    ...
    <VMenu placement="bottom-end" :distance="-1" v-if="subButtons.length > 0 && showMoreButton">
      <button
      ...
      <template #popper>
        <ol class=" flex flex-col gap-1.5 bg-white text-body-base rounded-3px rounded-tr-none shadow-xl p-6">
          <li v-for="button in subButtons">
            <a :href="button.link" class="font-bold transition-colors text-sub-menu hover:text-sub-menu-hover hover:underline">
              {{ button.label }}
            </a>
          </li>
        </ol>
