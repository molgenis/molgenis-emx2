<script setup>

function submitSearch() {
    alert('search')
};

</script>
<template>
    <form class="relative " @submit.prevent="submitSearch">
        <input type="search"
            class="w-full pr-16 font-sans text-gray-300 bg-white outline-none rounded-search-input h-10 ring-red-500 pl-3 border-search-input focus:border-white shadow-search-input focus:shadow-search-input hover:shadow-search-input"
            placeholder="Search cohorts" />
        <button
            class="rounded-search-button absolute top-0 right-0 flex items-center pl-8 pr-6 tracking-wider text-search-button border-search-button border-[1px] transition-colors bg-search-button h-10 font-display text-heading-xl hover:text-search-button-hover">
            Search
        </button>
    </form>
</template>