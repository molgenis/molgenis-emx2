...
<template>
  ...
  <div class="flex gap-1 p-5 items-center">
    <div class="inline-flex gap-1 group" @click="toggleCollapseTitle()">
      <h3
        class="text-white font-sans text-body-base font-bold mr-[5px] group-hover:underline group-hover:cursor-pointer">
        {{ title }}
      </h3>
      <span :class="{ 'rotate-180': collapsedTitle }"
        class="rounded-full group-hover:bg-blue-800 group-hover:cursor-pointer w-8 h-8 text-white flex items-center justify-center">
        <BaseIcon name="caret-up" width="26" />
      </span>
    </div>
    <div class="grow text-right">
      <span class="text-body-sm text-yellow-500 hover:underline hover:cursor-pointer">Remove 2 selected
      </span>
    </div>
  </div>
  ...
  <ul class="ml-5 mb-5 text-white" :class="{ hidden: collapsedTitle }">
    <li v-for="item in json" :key="item.code" class="mb-2.5">
      <div class="flex items-start">
        <span @click="toggleCollapse()" :class="{ 'rotate-180': collapsed }"
          class="text-white rounded-full hover:bg-blue-800 hover:cursor-pointer h-6 w-6 flex items-center justify-center">
          <BaseIcon name="caret-up" width="20" />
        </span>
        <div class="flex items-center">
          <input type="checkbox" :id="item.code" :name="item.code"
        ...
        <label :for="item.code" class="hover:cursor-pointer text-body-sm group">
          ...
          <div class="whitespace-nowrap inline-flex items-center">
            <span class="text-blue-200 inline-block mr-2 group-hover:underline decoration-blue-200 fill-black"
              :hoverColor="white">&nbsp;- 34
            </span>
