...
export default {
  ...
  methods: {
    ...
    loadSchema() {
      ...
      request(
        "graphql",
        "{_session{schemas,roles}_schema{name,tables{name,tableType,inherit,externalSchema,description,semantics,columns{name,table,position,columnType,inherited,key,refSchema,refTable,refLink,refBack,required,readonly,description,semantics,validation,visible,computed} } } }"
      )
        .then((data) => {
