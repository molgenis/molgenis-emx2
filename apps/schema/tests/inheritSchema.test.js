...
test("editing a grandchild keeps its own parent instead of the root table", async () => {
  const { parentTableOptions, resolveInheritNames } = await import(moduleUrl);
  const root = {
    name: "A",
    subclasses: [
      { name: "B", inheritName: "A" },
  ...
  assert.deepStrictEqual(resolveInheritNames(["B"], options), ["B"]);
});
...
test("editing a direct child keeps the root table as its parent", async () => {
  ...
  assert.deepStrictEqual(
    resolveInheritNames(["A"], parentTableOptions(root, "B")),
    ["A"]
  );
});

test("editing a diamond child keeps every parent that is still offered", async () => {
  const { parentTableOptions, resolveInheritNames } = await import(moduleUrl);
  const root = {
    name: "A",
    subclasses: [
      { name: "B", inheritName: "A" },
      { name: "C", inheritName: "A" },
      { name: "D", inheritNames: ["B", "C"] },
    ],
  };
  assert.deepStrictEqual(
    resolveInheritNames(["B", "C"], parentTableOptions(root, "D")),
    ["B", "C"]
  );
});

test("a parent that is no longer offered is dropped from a diamond child", async () => {
  const { resolveInheritNames } = await import(moduleUrl);
  assert.deepStrictEqual(
    resolveInheritNames(["B", "removedSubclass"], ["A", "B"]),
    ["B"]
  );
});
...
test("a new subclass extends the root table by default", async () => {
  ...
  assert.deepStrictEqual(
    resolveInheritNames(undefined, parentTableOptions(root, undefined)),
    ["A"]
  );
});
...
test("a root table without subclasses only offers itself as parent", async () => {
  ...
  assert.deepStrictEqual(resolveInheritNames(undefined, ["A"]), ["A"]);
});
...
test("a parent that is no longer offered falls back to the root table", async () => {
  const { resolveInheritNames } = await import(moduleUrl);
  assert.deepStrictEqual(resolveInheritNames(["removedSubclass"], ["A", "B"]), [
    "A",
  ]);
});
