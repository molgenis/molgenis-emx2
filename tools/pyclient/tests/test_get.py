...
def test_columns_okay():
    ...
    with Client(url=server_url) as client:
        ...
        pets_list = client.get(schema="pet store", table="Pet",
                               columns=["name", "weight", "orders"])
        assert list(pets_list[0].keys()) == ["name", "weight", "orders"]

        pets_df = client.get(schema="pet store", table="Pet",
                             columns=["name", "weight", "orders"], as_df=True)
...
def test_query_filter_fail():
    ...
    with Client(url=server_url) as client:
        ...
        assert exc_info.value.args[0] == ("Cannot process statement 'name',"
                                          " ensure specifying one of the"
                                          " operators '==', '>', '<', '!=',"
...
def test_equals_filter():
    ...
    with Client(url=server_url) as client:
        ...
        pets = client.get(table="Pet", schema="pet store",
                          query_filter="name == 'Henry'")
        assert len(pets) == 0

        pets = client.get(table="Pet", schema="pet store",
                          query_filter="name == 'pooky'")
        assert len(pets) == 1

        # Test int
        orders = client.get(table="Order", schema="pet store",
                            query_filter="quantity == 7")
        assert len(orders) == 1

        # Test float
        pets = client.get(table="Pet", schema="pet store",
                          query_filter="weight == 9.4")
        assert len(pets) == 1

        # Test ref
        pets = client.get(table="Pet", schema="pet store",
                          query_filter="category.name == cat")
        assert len(pets) == 3

        # Test ontology
        pets = client.get(table="Pet", schema="pet store",
                          query_filter="tags.name == red")
        assert len(pets) == 4
...
def test_greater_filter():
    ...
    with Client(url=server_url) as client:
        ...
        # Test int/long
        orders = client.get(table="Order", schema="pet store",
                            query_filter="quantity > 5")
        assert len(orders) == 1

        # Test float
        pets = client.get(table="Pet", schema="pet store",
                          query_filter="weight > 1.1")
        assert len(pets) == 5
...
def test_not_equals_filter():
    ...
    with Client(url=server_url) as client:
        ...
        # Test float
        orders = client.get(table="Pet", schema="pet store",
                            query_filter="weight != 1.337")
        assert len(orders) == 9
        ...
        with pytest.raises(NotImplementedError) as exc_info:
            client.get(table="Pet", schema="pet store",
                       query_filter="category != cat")
        assert str(exc_info.value) == ("The filter '!=' is not implemented"
                                       " for columns of type 'RADIO'.")

        # Test ontology
        with pytest.raises(NotImplementedError) as exc_info:
            client.get(table="Pet", schema="pet store",
                       query_filter="tags != 'red'")
        assert str(exc_info.value) == ("The filter '!=' is not implemented"
                                       " for columns of type 'ONTOLOGY_ARRAY'.")
...
def test_between_filter():
    ...
    with Client(url=server_url) as client:
        ...
        # Test int/long
        orders = client.get(table="Order", schema="pet store",
                            query_filter="quantity between [5, 10]")
        assert len(orders) == 1

        orders = client.get(table="Order", schema="pet store",
                            query_filter="quantity between [7.5, 10]")
        assert len(orders) == 0

        # Test float
        orders = client.get(table="Pet", schema="pet store",
                            query_filter="weight between [0, 2.5]")
        assert len(orders) == 5

        # Test string
        with pytest.raises(NotImplementedError) as exc_info:
            client.get(table="Pet", schema="pet store",
                       query_filter="name between [0, 5]")
        assert str(exc_info.value) == ("The filter 'between' is not implemented"
                                       " for columns of type 'STRING'.")
...
def test_as_df():
    ...
    with Client(url=server_url) as client:
        ...
        assert sorted(pets.columns.to_list()) == ['category', 'mg_draft',
                                                  'name', 'orders', 'photoUrls',
                                                  'status', 'tags', 'weight']
        assert len(pets['weight'].to_list()) == 10
