...
def test_table_fail():
    ...
    with Client(url=server_url) as client:
        ...
        assert exc_info.value.msg == (
            "Table 'Pets' not found in schema 'pet store'.")
...
def test_columns_okay():
    ...
    with Client(url=server_url) as client:
        ...
        pets_list = client.get_graphql(schema="pet store", table="Pet",
...
def test_query_filter_fail():
    ...
    with Client(url=server_url) as client:
        ...
        with pytest.raises(ValueError) as exc_info:
            client.get_graphql(table="Pet", schema="pet store",
                               query_filter="name")
        assert exc_info.value.args[0] == (
            "Cannot process statement 'name', ensure specifying one of the"
            " operators '==', '>', '<', '!=', 'between' in your statement.")
...
def test_equals_filter():
    ...
    with Client(url=server_url) as client:
        ...
        # Test int
        orders = client.get_graphql(table="Order", schema="pet store",
...
def test_not_equals_filter():
    ...
    with Client(url=server_url) as client:
        ...
        # Test float
        orders = client.get_graphql(table="Pet", schema="pet store",
...
def test_multiple_filters():
    ...
    with Client(url=server_url) as client:
        ...
        pets = client.get_graphql(
            table="Pet", schema="pet store",
