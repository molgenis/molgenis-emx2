...
def test_delete_records():
    ...
    with Client(url=server_url) as client:
        with pytest.raises(PermissionDeniedException) as exc_info:
            client.delete_records(table="Pet", schema="pet store",
                                  file=RESOURCES_DIR / "insert" / "Pet.csv")
        assert str(exc_info.value) == ("Message: Transaction failed:"
                                       " permission denied.\n")
        ...
        with pytest.raises(NoSuchSchemaException) as exc_info:
            client.delete_records(table="Pet",
                                  file=RESOURCES_DIR / "insert" / "Pet.csv")

        assert exc_info.value.msg == ("Select an existing schema for this"
                                      " operation.")
        ...
        assert str(exc_info.value) == ("No data to import. Specify a file"
                                       " location or a dataset.")
...
def test_format_fail():
    with Client(url=server_url) as client:
        ...
        with pytest.raises(ValueError) as exc_info:
            client.delete_records(table="Pet", schema="pet store",
                                  data=["pooky"])
        assert str(exc_info.value) == ("Cannot prepare row 'pooky'."
...
def test_delete_with_file():
    ...
    with Client(url=server_url) as client:
        ...
        tag_before = len(client.get_graphql(schema="pet store", table="Tag",
                                            columns=["name"]))
        client.save_table(table="Tag", schema="pet store",
        ...
        client.delete_records(schema="pet store" , table="Tag",
                              file=RESOURCES_DIR / "insert" / "Tag.csv")
