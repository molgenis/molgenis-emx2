...
def test_artifact_roundtrip_transform(hpc_client):
    ...
    # ---- Step 1+2+3: Create input artifact, upload file, commit ----
    ...
    committed_input = create_and_commit_managed_artifact(
        ...
        files={"measurements.csv": csv_bytes},
    )
    ...
    # Single-file tree hash == file hash, as committed by helper.
    assert committed_input["sha256"] == csv_sha256
    assert committed_input["size_bytes"] == len(csv_bytes)
    ...
    with deterministic_temp_dir(f"e2e-roundtrip-{job_id}") as tmpdir:
        ...
        manifest_text = (tmpdir / "manifest.txt").read_text()
        for line in manifest_text.strip().splitlines():
            expected_hash, fname = line.split()
            actual_hash = hashlib.sha256((tmpdir / fname).read_bytes()).hexdigest()
