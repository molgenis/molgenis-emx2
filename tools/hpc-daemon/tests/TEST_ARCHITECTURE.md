...
└── service/                  # NEW: behavior tests for service logic
    └── ArtifactServiceTest.java    # tree hash computation

backend/molgenis-emx2-webapi/src/test/java/org/molgenis/emx2/web/
└── HpcApiE2ETest.java         # API integration: claims, errors, immutability, stale workers
...
│   ├── test_daemon.py        # Orchestration scenarios
...
|----|-------------|-------|-----------|-------------|----------|
| REQ-JOB-STATE-001 | Jobs follow state machine: PENDING→CLAIMED→SUBMITTED→STARTED→{COMPLETED,FAILED,CANCELLED} | M | HpcJobStatusTest | test_contract | test_02 |
| REQ-JOB-STATE-002 | Terminal states (COMPLETED, FAILED, CANCELLED) have no outgoing transitions | M | HpcJobStatusTest | test_contract | — |
| REQ-JOB-STATE-003 | Invalid transitions MUST return 409 Conflict | M | HpcJobStatusTest | — | — |
...
| REQ-JOB-CANCEL-001 | Cancel from any non-terminal state → CANCELLED | M | HpcJobStatusTest | — | test_04 |
...
| REQ-JOB-IDEMPOTENT-001 | Duplicate transitions SHOULD return 200 OK | S | — | — | — |
...
| REQ-JOB-INPUT-001 | Inputs accept array of strings | M | — | test_backend | — |
| REQ-JOB-INPUT-002 | Inputs accept array of objects with artifact_id | M | — | test_backend | test_06 |
| REQ-JOB-INPUT-003 | Inputs accept named-reference objects | M | — | test_backend | — |

### Artifacts

| ID | Requirement | Level | Java Test | Python Test | E2E Test |
|----|-------------|-------|-----------|-------------|----------|
| REQ-ART-STATE-001 | Managed: CREATED→UPLOADING→COMMITTED | M | ArtifactStatusTest | test_contract | test_06 |
| REQ-ART-STATE-002 | External: REGISTERED→COMMITTED | M | ArtifactStatusTest | — | — |
| REQ-ART-STATE-003 | Stale CREATED/UPLOADING expire to FAILED | M | — | — | — |
| REQ-ART-HASH-001 | Single-file: SHA-256 of file bytes | M | ArtifactServiceTest ✓ | test_daemon | test_06 |
| REQ-ART-HASH-002 | Multi-file: tree hash SHA256(concat(sorted path:sha256)) | M | ArtifactServiceTest ✓ | test_daemon | — |
...
| REQ-ART-RESIDENCE-002 | POSIX: file:// path, symlink on staging | M | — | test_daemon | test_05 |
| REQ-ART-LOG-001 | Daemon uploads execution logs as type "log" | S | — | test_daemon | test_02 |
...
| REQ-AUTH-HEADER-001 | X-EMX2-API-Version required | M | ApiVersionTest | test_contract | — |
...
| REQ-PROTO-HATEOAS-001 | Responses include _links advertising legal actions | M | LinkBuilderTest | — | — |
| REQ-PROTO-HATEOAS-002 | Links absent for illegal transitions | M | LinkBuilderTest | — | — |
...
| REQ-DAEMON-PROGRESS-001 | Read last line of .hpc_progress.jsonl (NDJSON) | S | — | test_daemon | — |
| REQ-DAEMON-PROGRESS-002 | Validate/sanitize progress fields | S | — | test_daemon | — |
...
| REQ-DAEMON-OUTPUT-001 | Output files classified: logs vs output | M | — | test_daemon | — |
| REQ-DAEMON-UPLOAD-001 | Managed output uploaded + committed with tree hash | M | — | test_daemon | test_06 |
| REQ-DAEMON-UPLOAD-002 | POSIX output registered + committed with tree hash | M | — | test_daemon | test_05 |
