...
//   * ("name","status","category",List.of("name")) where category is a relationship to a table
// having
...
//
//    return
// field(dsl.select(field("row_to_json(conn)")).from(table(dsl.select(fields)).as("conn")))
//        .as(column.getColumnName());
//  }
//
...
//        Column c = table.getColumn(f.getField());
//
//        // suppose validation has been done elsewhere
...
//      throw new SqlGraphQueryException(
//          "Query only accept string or list type. E.g. 'name','tag',List.of('name'). Found: " +
// o);
//    return (String) o;
//  }
//
