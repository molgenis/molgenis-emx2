...
**Countable types (facet counts):** ONTOLOGY, ONTOLOGY_ARRAY, BOOL, RADIO, CHECKBOX, STRING_ARRAY, REF, REF_ARRAY, REFBACK
...
**Navigable AND countable types (checkbox + expandable caret, independent):** REF, REF_ARRAY, REFBACK
**Navigable AND countable types (checkbox + expandable caret, independent):** SELECT, MULTISELECT
**String-like (shown in picker by default):** STRING, TEXT, EMAIL, HYPERLINK, UUID, AUTO_ID
**Auto-defaulted on first load (ONTOLOGY/RADIO/SELECT only):** ONTOLOGY, ONTOLOGY_ARRAY, RADIO, SELECT
**Picker-selectable but NOT auto-defaulted:** REF, REF_ARRAY, REFBACK, BOOL, CHECKBOX, MULTISELECT, STRING, RANGE, and all other countable/range types
...
| ALL countable types → renders FilterTree | Column.vue | Column.spec.ts: "renders FilterTree for ONTOLOGY/BOOL/STRING_ARRAY"; "REF/REF_ARRAY/REFBACK renders FilterTree" | - |
...
| Filter with >25 ROOT options shows first 25 roots + "Show more" button; each click reveals up to 50 more roots (clamps to remaining count); when all visible button becomes "Show less" (resets view to 25) | filter/Tree.vue | Tree.spec.ts | visual check |
...
| REF/REF_ARRAY/REFBACK directly filterable via facet counts (distinct FK target rows as checkboxes); reuses RADIO/CHECKBOX fetchFlatGroupBy strategy + GROUP_BY_SATURATION_THRESHOLD=500 + "too many options, please search" hint | fetchCounts.ts | fetchCounts.spec.ts: "REF column uses fetchFlatGroupBy"; "REF_ARRAY uses fetchFlatGroupBy"; "REFBACK uses fetchFlatGroupBy"; "REF saturation ≥500" | visual check |
...
| REF/REF_ARRAY/REFBACK counts use `<col>_groupBy { count, <col> { <keyLeaf> } }` via fetchFlatGroupBy with useGroupByField=true; same composite-key + saturation behavior as RADIO/CHECKBOX REF | fetchCounts.ts | fetchCounts.spec.ts: "REF column uses fetchFlatGroupBy"; "REF refLabelDefault template" | - |
| REF/REF_ARRAY/REFBACK single-key direct filter: `equals: [{ key: val }, ...]` shape; composite-key direct filter: `_or:[{key:{equals:val}},...]` | buildGqlFilter.ts | buildGqlFilter.spec.ts: REF single-key/composite; REF_ARRAY/REFBACK single-key | - |
| REF/REF_ARRAY/REFBACK with refTableId: flat URL key (no `.name` suffix), MULTI_VALUE_TYPE plain strings on parse | filterUrlParams.ts | filterUrlParams.spec.ts: REF/REF_ARRAY/REFBACK flat serialization + roundtrip | - |
...
| REF/REF_ARRAY/REFBACK rows show BOTH a checkbox AND an expand caret + "→ tableName"; checkbox and caret are independent (label click = toggle checkbox; caret click only = expand) | Picker.vue | Picker.spec.ts: "REF columns show BOTH a checkbox AND an expand arrow"; "label click on REF row toggles checkbox" | visual check |
| REF expands 2 levels deep, REF_ARRAY/REFBACK 1 level deep | Picker.vue | Picker.spec.ts | - |
| Selecting REF in Picker adds a sidebar section with checkboxes of distinct FK target rows + counts (separate from nested-child filters on same ref; AND'd together) | Picker.vue, Sidebar.vue | Picker.spec.ts: "ONTOLOGY/BOOL/REF checkboxes shown by default"; filter-sidebar.spec.ts (e2e) | visual check |
