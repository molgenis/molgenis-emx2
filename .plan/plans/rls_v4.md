...
- **R.7** — Shrink `PermissionSet.ReferenceScope` to `{NONE, ALL}`. Owner direction (2026-05-10): VIEW⊇REFERENCE implicit carry already covers self/group cases; REFERENCE_ALL alone has a real use case (lookup tables). Drop the rest to avoid dead-code optionality. _Status (2026-05-10): DONE._ `PermissionSet.java` −2 lines (dropped OWN/GROUP enum constants). `migration32.sql` −5 lines net: narrowed two CHECK constraints from `('NONE','OWN','GROUP','ALL')` to `('NONE','ALL')`, and dropped the two `OR (rp.reference_scope = …)` SQL branches from `mg_can_reference` body. `GraphqlPermissionFieldFactory.java` unchanged — the enum builder iterates `ReferenceScope.values()` dynamically so the GraphQL surface auto-narrowed. Tests updated: `TestSqlRoleManager` renamed `referenceScope_roundTrip_ownGroupNone` → `referenceScope_roundTrip_none`; `TestMgCanReference` −44 lines (deleted `mgCanReference_returnsTrue_whenReferenceOwn_andUserIsOwner` and `_whenReferenceGroup_andUserInGroup`); `TestGraphqlPermissionFieldFactory` swapped two enum-value assertions from OWN/GROUP to ALL. Docs + spec updated to reflect `{NONE, ALL}` shape; v1-caveat sentence removed; combination matrix row `VIEW_GROUP + REFERENCE_ALL` collapsed to `VIEW_GROUP + REFERENCE_NONE`. Storage column stays `TEXT NOT NULL DEFAULT 'NONE'` (no migration; just narrowed value domain). Tests green: TestSqlRoleManager 31/31, TestMgCanReference 6/6, TestGraphqlPermissionFieldFactory 19/19, TestGraphqlSchemaRoles 33/33 no regression, TestCrossSchemaFkRlsVisibility 10/10 no regression, TestFkRlsWriteGuard 11/11 no regression.
...
- **Scoped REFERENCE (REFERENCE_GROUP / REFERENCE_OWN)** — ~~ANSWERED: deferred to a later phase. v1 wires only NONE and ALL. The enum carries all four values (API prepped) so promotion to GROUP/OWN is purely a runtime change.~~ **REVISED 2026-05-10**: dropped entirely. Standalone REFERENCE_OWN/GROUP adds no expressiveness beyond VIEW_OWN/GROUP via the implicit-carry rule (VIEW ⊇ REFERENCE at same scope tier); the only useful REFERENCE scope is ALL (lookup-table pattern). Shrinking enum to `{NONE, ALL}` removes dead branches from `mg_can_reference`, simplifies docs, and avoids "what does REFERENCE_OWN even mean" confusion. Tracked as R.7.
