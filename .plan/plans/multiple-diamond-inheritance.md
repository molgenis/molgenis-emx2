...
- [ ] **FOLLOW-UP (flagged 2026-07-08, NOT yet fixed) — verify migration32.sql idempotency on a genuinely fresh DB.** The H.3 root-cause agent hit `migration32.sql`'s `ALTER COLUMN table_inherits TYPE VARCHAR[] USING ARRAY[table_inherits]` failing when `table_inherits` was ALREADY `VARCHAR[]` (baseline `MetadataUtils.init()` declares it as such) — the `ARRAY[...]` wrap assumes a scalar source. **Likely an ARTIFACT** of a concurrent unrelated worktree (`feat/details_view_like_catalogue`) resetting the SHARED dev Postgres to a pre-migration32 intermediate state mid-session (repaired via direct SQL, no source change) — a truly fresh install normally sets the version to current and does NOT re-run migration32, and this branch's fresh 5434 provisioning + `TestMigration.testMigration32` (E5) + all suites have been green. Backend agent: red-green confirm BOTH paths — (a) genuinely fresh DB create, (b) pre-v33 scalar DB upgraded via migration32 — migrate cleanly; guard the ALTER with a scalar-vs-array check only if a real fresh-DB failure reproduces. Low priority; strongly reinforces the value of the 5434 dev-DB isolation (`.env` / PR #6466) since the trigger was cross-worktree shared-DB interference.
- [ ] **ROADMAP (BIG, owner 2026-07-08) — new schema editor: separate inheritance from module-polymorphism + solve column order + derive a YAML format.** Owner flagged as a major roadmap effort (an EXPANSION of F5, likely its own initiative, not part of the current diamond PR). Three intertwined threads:
  1. **Two orthogonal axes, presented distinctly.** The new editor must clearly SEPARATE **traditional (multiple/diamond) inheritance** (`extends` / is-a / `mg_tableclass`) from **module-based polymorphism** (`MODULE`/`MODULE_ARRAY` composition). Today they share the `extends`+`tableType` machinery under the hood (Design B), but to a modeler they are different concepts (an is-a subtype vs a composable module/variant) and the UI should not conflate them.
  2. **Column ORDER is the sore point.** The current BOOTSTRAP schema editor (`apps/schema`) manages column order well — EXCEPT under **cross-schema inheritance**, which has great use cases and currently breaks ordering. The new editor must make column order controllable INCLUDING across schema boundaries (relates to E6/H single-file cross-module display order via `position`/row-order, but here the harder case is cross-SCHEMA parents). This is the owner's stated #1 pain ("what really bugs me").
  3. **Derive a YAML format from the editor's model.** A suitable YAML schema format derived from the new editor (bidirectional model ↔ YAML). **Owner has other worktrees already exploring options:** `mswertz/feat/yaml_profile_format`, `mswertz/poc/linkml_mapping_yaml_format` (LinkML mapping), `mswertz/poc/datamodel_pilot1` — reconcile/pull from those rather than starting fresh.
  Relation to current work: supersedes/absorbs the narrow Phase **F5** (schema-editor MODULE/MODULE_ARRAY/diamond authoring) into a larger editor rethink. NOT scheduled into the diamond PR — capture-only until the owner promotes it. Owner of execution: TBD (frontend + a design pass; datamodeler for the YAML mapping).
  - **OPEN QUESTION (owner 2026-07-08): distinct inheritance modes for WITHIN-schema vs ACROSS-schema?** Owner: "should we have distinct inheritance modes for 'within schema' and 'across schema'? … naive molgenis.csv and thus also YAML even when multiple files will be schema scoped." Rationale: file formats (CSV/YAML) are SCHEMA-SCOPED, so a within-schema `extends` is a LOCAL edge (parent's columns are in the same file/position-space → full column-order control) while a cross-schema `extends` is an EXTERNAL reference (parent lives in another schema → its columns are referenced, not owned → the column-order breakage the owner hit is inherent to spanning position-spaces). **This directly explains the column-order pain:** the bug is trying to interleave a foreign parent's columns by a shared `position` that doesn't span schemas. **Lead's recommendation (for owner review, NOT settled):** DERIVE the distinction in the ENGINE (parent-in-this-schema? — no new column type; storage is identical table-per-type on shared root PK, so a hard engine mode adds complexity for a derivable fact) but make it EXPLICIT in the FORMAT + EDITOR layer — a cross-schema parent renders as an `imports:`-style external reference (ordered block, not position-interleaved), a within-schema parent stays a local editable edge. Note the existing asymmetry this would make symmetric: module composition (`MODULE`/`MODULE_ARRAY`) is ALREADY same-schema-only (bare names); is-a can currently be either (schema-qualified `schema.Table`). So the 2×2 is {is-a, composition} × {within, across}, with cross-schema composition currently DROPPED (Design B, "reversible later"). Feeds the YAML format (LinkML `imports` + `is_a`/`mixins`) and the editor's local-vs-foreign parent rendering.
