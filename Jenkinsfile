    ...
    stages {
        ...
        stage("Pull request") {
            ...
            steps {
                container('java') {
                    script {
                    sh "./gradlew checkFormat spotlessCheck test --no-daemon jacocoMergedReport shadowJar jib release helmPublishMainChart ci \
        ...
        stage('Master') {
            ...
            steps {
                container('java') {
                    script {
                        sh "./gradlew test checkFormat spotlessCheck --no-daemon jacocoMergedReport shadowJar jib release helmPublishMainChart sonarqube ci \
                            -Dsonar.login=${SONAR_TOKEN} -Dsonar.organization=molgenis -Dsonar.host.url=https://sonarcloud.io \
                            -Dorg.ajoberstar.grgit.auth.username=${GITHUB_TOKEN} -Dorg.ajoberstar.grgit.auth.password"
                        def props = readProperties file: 'build/ci.properties'
                        env.TAG_NAME = props.tagName
