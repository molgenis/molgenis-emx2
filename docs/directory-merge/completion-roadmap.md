...
| `parent_collection` (1,319 sub-collections / 206 parents) | classify by varying dimension: facts-dims → `Collection facts` (default, **934 / 71%**); `type` varies → promoted `Collections` + `Linkages`; temporal → `Collection events`; site (name-coded) → `Subpopulations` | 385 (29%) reviewed: `collection_event` 38 / `subpopulation` ~92 (site-arms, name-coded) / `promoted_collection` 35 / `unclear_mixture` ~220 |
...
The per-sub-collection analysis (1,319 sub-collections across 206 parents) classifies **934 (71%) → `Collection facts`** automatically; **385 (29%)** need review — `collection_event` 38, `subpopulation` ~92 (site-arms, see below), `promoted_collection` 35, `unclear_mixture` ~220. The refinements below lift the auto-rate and tame the review queue:
...
- **`subpopulation` IS used — but name-coded (corrected finding).** ~92 of the 111 `name_only_slice` sub-collections are **site-arms**: disease cohorts split per recruiting Dutch academic hospital (`DIA-AMC` / `DIA-UMCG` / `DIA-VUMC` … across 14 parents). The initial parent-grain pass reported `subpopulation` 0 only because its site regex matched English words (`centre|hospital`), not institutional **abbreviations**. So site/subpopulation detection must be **abbreviation-aware** — ~92 sub-collections map to `Subpopulations`, not `unclear`. (Supersedes the earlier "subpopulation unused" note.)
...
- **`unclear_mixture` (~220) review queue** breaks down as: ~38 duplicates + ~19 name-only slices (distinction only in the free-text name, no structured column — the ~92 site-arm name-slices now go to `Subpopulations`) + ~83 facts×wave + ~34 near-facts (a spurious `data_categories` diff — a curator would likely restore to facts) + ~46 single-child parents (flatten into parent).
