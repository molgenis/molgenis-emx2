...
- **[mapping_ledger.csv](mapping_ledger.csv) — the case-by-case mapping of every directory record** (29,013 rows: one per source record + minted legal entities). Columns: `directory_table`, `directory_id`, `directory_name`, `catalogue_table`, `catalogue_id`, `mapping_rule`, `disposition` (`auto` / `needs_curation` / `dropped`), `flag_reason`, `from_UMCG`, `data_quality_error`, `data_quality_reason`, `collections_fold_review`, `name_coherence`, `part_of_possible_facts_fold`. Headline **94% auto / 3.2% needs-curation / 2.9% dropped**. **`data_quality_error`/`data_quality_reason`** flag where a record violates a BBMRI-ERIC Directory Manual §5 data-quality check (e.g. `juridical_person` is a person not a legal entity, placeholder/blank content, malformed IDs, bad geo-coords) — **~1,084 rows (3.7%) flagged**; see [data_quality_summary.md](data_quality_summary.md). **`collections_fold_review` / `name_coherence`** (on biobank rows) — whether a multi-collection biobank's collections are actually facts slices (`collections_are_facts` / `mixed` / `distinct`) + a confidence signal (low = verify by hand, e.g. `NL_UMCG`); **`part_of_possible_facts_fold`** (on collection rows) flags collections under such a biobank. **Advisory** — a review signal, not a settled mapping (`disposition` unchanged). Filter `disposition = needs_curation` for the curation worklist, or `from_UMCG = TRUE` for fix-at-source. Summary in [mapping_ledger_summary.md](mapping_ledger_summary.md).
