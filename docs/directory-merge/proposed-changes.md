...
| # | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|---|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| C1 | **New `Organisations extends Resources`** = top level identity (`foaf:Agent`). Own key (name/pid); `ror id` reference; inherits `source`/provenance; address/contact. A **biobank is an `Organisations` row here** (optionally with operation attributes and linked `Services`/`QualityInfo`). No separate `Biobanks` subtype is required but could be optionally created.                                                                               |
...
| C7 | **Networks & Catalogues stay Resource containers** (option 2). A network/catalogue's maintainer is its existing `publisher` (→ Organisations). A directory Network → a `Networks` container **plus** an `Organisations` row for the coordinating body.                                                                                                                                                                                                   |
| C8 | **Federation**: add `source` + a namespaced identifier scheme on `Resources` (R5/R6): `source` = owning catalogue, global id = `source:localid`, never rewritten on merge.                                                                                                                                                                                                                                                                               |
...
- **Biobank** → an `Organisations` identity; its datasets → `Collections` **held by** it.
...
- **`Biobanks` subtype** — not needed now; reintroduce only if biobank-operation attributes proliferate beyond a few optional columns + linked `Services`/`QualityInfo`.
