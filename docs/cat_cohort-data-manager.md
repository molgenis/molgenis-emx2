...
- Similarly, the common data model (or 'target variables') can be considered the codebook of a network of cohorts working together (e.g. LifeCycle)
...
It is good practice to try out adding a few variables to the template first and see whether your upload succeeds. To upload the metadata to the Data Catalogue see the section [Upload metadata](cat_cohort-data-manager.md#upload-metadata) to the Data Catalogue.
...
After the correct authentication and authorisation steps have been set up researchers will be able to analyse the cohort’s harmonised data via DataSHIELD.
