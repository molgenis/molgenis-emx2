SELECT json_build_object(
  ...
  'dct:publisher', COALESCE(
    (
      SELECT json_build_object(
        '@id', ${base_url} || '/' || ${schema} || '/api/organisation/' || o.id,
        '@type', 'foaf:Agent',
        'foaf:name', o.name
      )
      FROM "Organisations" o
      WHERE o.id = (SELECT unnest(r."lead organisation") LIMIT 1)
    ),
    json_build_object(
  ...
  'dcat:dataset', COALESCE(
    (
      SELECT json_agg(json_build_object(
        '@id', ${base_url} || '/' || ${schema} || '/api/fdp/dataset/' || d.name
      ))
      FROM "Datasets" d
      WHERE d.resource = r.id
    ),
    '[]'::json
  )
) AS result
FROM "Extended resources" r
WHERE r.id = ${id}
