...
public class SqlDatabase implements Database {
  ...
  private Condition createConditions(String name, Query q) {
    ...
    for (Where w : q.getWhereLists()) {
      ...
      for (Object o : w.getValues()) {
        if (c == null) c = field(name(tableAlias, fieldName)).eq(o);
        else c = c.and(field(name(tableAlias, fieldName)).eq(o));
      }
    }
    return c;
  }
  ...
  public SelectJoinStep createJoins(String name, Query q, Table table, SelectJoinStep from)
    ...
    for (Select s : q.getSelectList()) {
      ...
      if (path.length >= 2) {
        ...
        String leftColumn = rightPath[rightPath.length - 1];
        ...
        if (!duplicatePaths.contains(rightAlias)) {
          ...
          from.leftJoin(table(name(c.getRefTable().getName())).as(name(rightAlias)))
