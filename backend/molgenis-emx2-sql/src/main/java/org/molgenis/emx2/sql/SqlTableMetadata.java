...
class SqlTableMetadata extends TableMetadata {
            ...
            db -> {
              ...
              try {
                ...
                if (enabled && !currentlyEnabled) {
                  ...
                  for (Role role : MetadataUtils.listRoles(jooq)) {
  ...
  private static void reEmitVerbIfOwnOrGroup(
      DSLContext jooq, String pgRole, String schemaName, String tableName, Permission p) {
    ...
    for (Role role : MetadataUtils.listRoles(jooq)) {
      ...
      for (Permission p : existing) {
        if (p.schema().equals(schemaName) && p.table().equals(tableName)) {
          if (p.select() == Permission.ViewScope.OWN
