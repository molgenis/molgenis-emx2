...
//        schema.add(column(TABLE_SCHEMA).pkey(true));
//
...
//        table.setPrimaryKey(TABLE_SCHEMA, TABLE_NAME);
//
...
//        column.setPrimaryKey(TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME); //should be foreign key too with cascade delete!
//
...
//        unique.setPrimaryKey(TABLE_SCHEMA,TABLE_NAME, UNIQUE_COLUMNS); //implicit also true
//
//        TableMetadata users = new TableMetadata(USERS_METADATA);
//        users.add(column(USER_NAME).pkey(true));
...
//        db.getSchema(MOLGENIS).getTable(SCHEMA_METADATA).update(new Row().set(TABLE_SCHEMA, TABLE_SCHEMA));
//
//    }
//
//    protected static void deleteSchema(Database db, SchemaMetadata schema) {
//        db.getSchema(MOLGENIS).getTable(SCHEMA_METADATA).delete(new Row().set(TABLE_SCHEMA, TABLE_SCHEMA));
//    }
//
//    protected static Collection<String> loadTableNames(Database db, SchemaMetadata sqlSchema) {
//        List<Row> result = db.getSchema(MOLGENIS).query(TABLE_METADATA).filter(TABLE_SCHEMA, Operator.EQUALS, sqlSchema.getName()).getRows();
...
//        List<Row> result = db.getSchema(MOLGENIS).query(TABLE_METADATA).filter(TABLE_SCHEMA, Operator.EQUALS, table.getSchema().getName()).filter(TABLE_NAME, Operator.EQUALS, table.getTableName()).getRows();
//        if (result.size() != 1) {
//            return;
//        }
//        table.setInherit(result.get(0).getString(TABLE_INHERITS));
//        table.setDescription(result.get(0).get(TABLE_DESCRIPTION, String.class));
...
//        row.set(PKEY, column.isPrimaryKey());
...
//        List<Row> results = db.getSchema(MOLGENIS).query(SCHEMA_METADATA).filter(TABLE_SCHEMA, Operator.EQUALS, name).getRows();
//        return results.size() == 1;
//    }
//
//    protected static void loadUniqueMetadata(Database db, TableMetadata table) {
//        List<Row> result = db.getSchema(MOLGENIS).query(TABLE_SCHEMA).filter(TABLE_SCHEMA, Operator.EQUALS, table.getSchema().getName()).filter(TABLE_NAME, Operator.EQUALS, table.getTableName()).getRows();
//        for (Row r : result) {
...
//
//        Query q = db.getSchema(MOLGENIS).query(COLUMN_METADATA)
...
//        db.getSchema(MOLGENIS).getTable(USERS_METADATA).update(new Row().set(USER_NAME, user).set(password, password));
//    }
//
//    public static boolean checkUserPassword(Database db, String username, String password) {
//        List<Row> result = db.getSchema(MOLGENIS).query(USERS_METADATA).filter(USER_NAME, Operator.EQUALS, username).getRows();
