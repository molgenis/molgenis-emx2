...
class SqlColumnRefArrayExecutor {
  ...
  private static void createIsReferencedByTrigger(DSLContext jooq, Column column) {
    if (column.isCompositeRef()) {
      throw new MolgenisException("Unsupported", "not yet implemented");
    } else {
      ...
      Name toColumn = name(column.getRefColumns().get(0).getName());
      ...
      jooq.execute(
          ...
          inline(column.getRefColumns().get(0).getName()));
    ...
    if (column.isCompositeRef()) {
      ...
      jooq.execute(
          "CREATE FUNCTION {0}() RETURNS trigger AS"
              ...
              + "\n$BODY$ LANGUAGE plpgsql;",
          functionName, // {0}
          keyword(SqlTypeUtils.getPsqlType(column.getRefColumns().get(0)) + "[]"), // {1}
          thisColumn, // {2}
          toColumn, // {3}
          toTable, // {4}
          ...
          inline(column.getRefColumns().get(0).getName())); // {8}
