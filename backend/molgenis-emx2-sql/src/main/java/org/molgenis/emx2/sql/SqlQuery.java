...
public class SqlQuery extends QueryBean {
  ...
  public String retrieveJSON() {
    ...
    if (logger.isInfoEnabled()) {
      ...
      logger.info("query created in {}ms", System.currentTimeMillis() - start, select.getColumn());
    }
  ...
  private static @NotNull String getRefTableAlias(SelectColumn select, String tableAlias) {
  ...
  public SelectJoinStep<Record1<Object>> getQueryForField(
      SqlTableMetadata table,
      Column parentColumn,
      String parentAlias,
      SelectColumn select,
        ...
        aggregateFields.add(
            jsonObject(
                    selectedField.getSubselect().stream()
                        .map(
                            sub -> {
                              Column subColumn = getColumnForGraphqlField(table, sub);
                              Field subField =
                                  field(
        ...
        if (subColumn.isReference()) {
          ...
          if (tableAlias.endsWith("_groupBy")
            ...
            if (subColumn.isRefArray()) {
              // in case of group by we must unnest the ref_array and join against that
              ...
              refJoinFields.addAll(
                  unnestFields.stream()
                      .map(field -> field(name(fieldName, "_parent_" + field.getName())))
                      .toList());
              unnestedRefArrayJoins.add(
                  // unnest from tableAlias
                  unnest(
                          unnestFields.stream()
                              .map(field -> field(name(alias(tableAlias), field.getName())))
                              .toList())
                      // into fieldName(col1,col2)
                      .as(
                          fieldName,
                          unnestFields.stream()
              ...
              // refback will cross join against primary key
              refJoinFields.addAll(
            ...
            refSubSelects.put(
                ...
                getQueryForField(
                    ...
                    new String[0]));
          } else if (tableAlias.endsWith("_groupBy") && subColumn.isRefback()) {
            // refback will use primary key
            refJoinFields.addAll(
                table.getPrimaryKeyFields().stream()
                    .map(ref -> field(name(alias(tableAlias), ref.getName())).as(ref.getName()))
                    .toList());
          } else {
            dataFields.add(
                getQueryForField(
  ...
  private Condition refJoinCondition(
      Column column, String tableAlias, String subAlias, String parentPrefix) {
      ...
      if (column.getReferences().size() == 1) {
        ...
        foreignKeyMatch.add(
            field(name(alias(subAlias), ref.getRefTo()))
                .eq(field(name(alias(tableAlias), parentPrefix + ref.getName()))));
      } else {
        foreignKeyMatch.add(
            and(
                ...
                or(
                    column.getReferences().stream()
                        .map(
                            ref ->
                                field(name(alias(tableAlias), parentPrefix + ref.getName()))
                                    .isNotNull())
                        .toList()),
                        ...
                        .map(
                            ref ->
                                field(name(alias(subAlias), ref.getRefTo()))
                                    .eq(
                                        field(
                                            name(alias(tableAlias), parentPrefix + ref.getName()))))
                        .toList())));
      }
        ...
        List<Field<Object>> unnest =
            column.getReferences().stream()
                .map(
                    r ->
                        r.isOverlappingRef()
                            ? field(name(alias(tableAlias), parentPrefix + r.getName()))
                            : field(UNNEST_0, name(alias(tableAlias), parentPrefix + r.getName())))
                .toList();
                ...
                .map(
                    ref ->
                        field(name(alias(subAlias), ref.getName()))
                            .eq(field(name(alias(tableAlias), parentPrefix + ref.getRefTo()))))
                .toList());
                ...
                .map(
                    ref ->
                        ref.isOverlappingRef()
                            ? field(name(alias(tableAlias), parentPrefix + ref.getRefTo()))
