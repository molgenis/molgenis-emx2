...
//  public static int AGGREGATE_COUNT_THRESHOLD = Integer.MIN_VALUE; // threshold disabled by
// default
...
//      "Operator BETWEEEN a AND b expects even number of parameters to define each pair of a,b.
// Found: %s";
//
...
//              "Select of column '"
//                  + column.getName()
//                  + "' failed: composite foreign key requires subselection or explicit naming of
// underlying fields");
//        } else {
//          fields.add(
//              field(name(alias(tableAlias), column.getName()),
// ref.getJooqType()).as(columnAlias));
//        }
...
//      table =
//          schema.getTableMetadata(select.getColumn().substring(0, select.getColumn().length() -
// 4));
...
//      fields.add(key(convertToPascalCase(select.getColumn())).value(
//          jsonAggregateSelect(
...
//      }
//      fields.add(key(convertToPascalCase(select.getColumn())).value(
//          jsonSubselect(table, null, table.getTableName(), select, getFilter(),
// getSearchTerms())));
//    }
//
//    // asemble final query
...
//    if (logger.isInfoEnabled()) {
//      logger.info(
//          "query in {}ms: {}", System.currentTimeMillis() - start,
// query.getSQL(ParamType.INLINED));
//    }
//    return result;
//  }
//
//  private JSONEntry<?> jsonSubselect(
...
//    Collection<JSONEntry<?>> selection = jsonSubselectFields(table, subAlias, select);
//    return jsonField(
//        table, parentColumn, tableAlias, select, filters, searchTerms, subAlias, selection);
//  }
//
//  private JSONEntry<?> jsonField(
//      SqlTableMetadata table,
//      Column column,
//      String tableAlias,
//      SelectColumn select,
...
//      String[] searchTerms,
//      String subAlias,
//      Collection<JSONEntry<?>> selection) {
...
//      from =
// jooq.select(jsonObject(selection)).from(tableWithInheritanceJoin(table).as(alias(subAlias)));
//    } else {
//      //list of json objects is to be expected
//      from =
// jooq.select(jsonArrayAgg(jsonObject(selection))).from(tableWithInheritanceJoin(table).as(alias(subAlias)));
//    }
...
//    }
//    //return a JSONEntry
//    return key(select.getColumn()).value(from);
//  }
//
...
//          conditions.add(
//              and(jsonFilterQueryConditions(table, column, tableAlias, subAlias, f,
// searchTerms)));
...
//                // normal ref
//                List<Field> refs =
// c.getReferences().stream().map(Reference::getJooqField).toList();
...
//                  // include if array overlap between ontology table and our selected values in
// our
//                  // ref_array
//                  search.add(
//                      condition(
...
//                                      .likeIgnoreCase("%" + term + "%"))));
//                } else {
//                  // include if our ref is in the ontology terms list that would be found given
// our
//                  // search terms
//                  search.add(
//                      ontologyColumn
...
//  }
//
//  private Collection<JSONEntry<?>> jsonSubselectFields(
...
//        fields.add(key(convertToCamelCase(select.getColumn())).value(
//            jsonAggregateSelect(
...
//      } else {
//        // primitive fields
//        fields.add(key(column.getIdentifier()).value(
//            field(name(alias(tableAlias), column.getName()))));
//      }
//    }
//    return fields;
...
//                      field(name(alias(tableAlias), column.getName())))));
//        } else {
//          subFields.add(key(ext).value(field(name(alias(tableAlias), column.getName() + "_" +
// ext)).as(ext)));
//        }
//      }
//    }
//    return key(column.getIdentifier()).value(jsonObject(subFields));
//  }
//
...
//        } else {
//          fields.add(key(COUNT_FIELD).value(field("GREATEST(COUNT(*),{0})", 10L)));
//        }
...
//          }
//        }
//        fields.add(jsonObject(result.toArray(new
// JSONEntry[result.size()])).as(field.getColumn()));
//      }
//    }
//    return jsonField(table, column, tableAlias, select, filters, searchTerms, subAlias, fields);
//  }
//
//  private JSONEntry<Object> jsonGroupBySelect(
//      SqlTableMetadata table,
//      Column column,
//      String tableAlias,
...
//    }
//
//    List<JSONEntry<?>> aggregationFields = new ArrayList<>(); // sum(x), count, etc
//    Set<JSONEntry<?>> groupByFields = new HashSet<>(); // name, ref{otherName}, etc
//    Set<JSONEntry<?>> nonArraySourceFields = new HashSet<>(); // xo x, name, except those from
// ref_array
...
//      if (COUNT_FIELD.equals(field.getColumn())) {
//        if (schema.hasActiveUserRole(VIEWER.toString())) {
//          aggregationFields.add(key(COUNT_FIELD).value(field("COUNT(*)")));
//        } else {
//          aggregationFields.add(key(COUNT_FIELD).value(
//              field("GREATEST({0},COUNT(*))", AGGREGATE_COUNT_THRESHOLD)));
//        }
...
//                  nonArraySourceFields.add(key(col.getName()).value(col.getJooqField()));
//                });
//        aggregationFields.add(key(field.getColumn()).value(jsonObject(sumFields)));
//      } else {
...
//          groupByFields.add(key(convertToCamelCase(field.getColumn())).value(
//              jsonSubselect(
...
//                  ));
//        } else {
//          groupByFields.add(key(convertToCamelCase(field.getColumn())).value(col.getJooqField()));
//        }
//
//        if (col.isRef() || !col.isArray()) {
//          nonArraySourceFields.addAll(col.getCompositeFields().stream().map(f ->
// key(f.getName()).value(f)).toList());
//        } else if (col.isRefback()) {
//          // convert so it looks like a ref_array
//          List<JSONEntry<?>> subselectFields = new ArrayList<>();
//          if (col.getRefBackColumn().isRefArray()) {
//            subselectFields.addAll(
//                col.getRefBackColumn().getReferences().stream()
//                    .map(ref -> key(ref.getRefTo()).value(field("unnest({0})",
// name(ref.getName()))))
//                    .toList());
//          } else {
//            subselectFields.addAll(
//                col.getRefBackColumn().getReferences().stream()
//                    .map(ref -> key(ref.getRefTo()).value(field(name(ref.getName()))))
//                    .toList());
//          }
//          subselectFields.addAll(
//              col.getReferences().stream()
//                  .map(ref -> key(ref.getName()).value(field(name(ref.getRefTo()))))
//                  .toList());
//
//          refArraySubqueries.add(
//                      jooq.select(jsonObject(subselectFields))
//                          .from(
...
//          List<JSONEntry<?>> subselectFields = new ArrayList<>();
//          subselectFields.addAll(table.getPrimaryKeyFields().stream().map(f ->
// key(f.getName()).value(f)).toList());
//          for (Field compositeField : col.getCompositeFields()) {
//            subselectFields.add(key(compositeField.getName()).value(field("unnest({0})",
// compositeField)));
//          }
//          refArraySubqueries.add(
//              jooq.select(jsonObject(subselectFields))
...
//    nonArraySourceFields.addAll(table.getPrimaryKeyFields().stream().map(f ->
// key(f.getName()).value(f)).toList());
//    SelectJoinStep<org.jooq.Record> sourceQuery =
//                jooq.select(jsonObject(nonArraySourceFields))
//                    .from(tableWithInheritanceJoin(table).as(alias(tableAlias)))
//                    .where(condition));
...
//    groupByFields.forEach(field -> orderByFields.add(field.key()).asc().nullsLast()));
//
//    // aggregate into one field
//    List<JSONEntry<?>> selectFields = new ArrayList<>();
//    selectFields.addAll(aggregationFields);
//    selectFields.addAll(groupByFields);
//    return field(
//            jooq.select(jsonArrayAgg(jsonObject(selectFields)))
...
//          conditions.add(
//              condition(
//                  "0 < ( SELECT COUNT(*) FROM unnest({1}) AS v WHERE word_similarity({0},v) >
// 0.6",
//                  value, field(columnName)));
//          break;
//        case TEXT_SEARCH:
//          conditions.add(
//              condition(
//                  "0 < ( SELECT COUNT(*) FROM unnest({1}) AS v WHERE to_tsquery({0}) @@
// to_tsvector(v)",
...
//            throw new SqlQueryException(
//                SqlQuery.QUERY_FAILED + SqlQuery.BETWEEN_ERROR_MESSAGE,
// TypeUtils.toString(values));
...
//      TableMetadata table, SelectColumn select, SelectConnectByStep<?> query) {
...
//      }
//      throw new MolgenisException(
//          "Query failed: Column '" + columnName + "' is unknown in table " +
// table.getTableName());
//    }
//    return column;
//  }
