  ...
  static void createRefBackColumnConstraints(DSLContext jooq, Column column) {
    if (column.isCompositeRef()) {
      ...
      try {
        ...
        Column toColumn = column.getRefColumns().get(0);
  ...
  private static void createTriggerForRefArray(DSLContext jooq, Column column) {
    if (column.isCompositeRef()) {
      ...
      jooq.execute(
          ...
          keyword(SqlTypeUtils.getPsqlType(column.getRefColumns().get(0))), // {1} type of item
          ...
          inline(column.getRefColumns().get(0).getName())); // {12} inline table name
