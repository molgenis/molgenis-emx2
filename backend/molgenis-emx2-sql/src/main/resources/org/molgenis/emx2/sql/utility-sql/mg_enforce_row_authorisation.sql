        ...
        FOR raw_role IN SELECT unnest(caller_roles) LOOP
            IF EXISTS (
                ...
                WHERE schemaname = TG_TABLE_SCHEMA
                  AND tablename  = TG_TABLE_NAME
                  AND policyname IN (
                      'MG_P_' || raw_role || '_SHARE_ALL',
                      'MG_P_' || raw_role || '_SHARE_GROUP',
                      'MG_P_' || raw_role || '_SHARE_OWN'
                  )
                  AND (
                      policyname = 'MG_P_' || raw_role || '_SHARE_ALL'
                      OR (policyname = 'MG_P_' || raw_role || '_SHARE_OWN'
                          AND OLD.mg_owner = session_user)
                      OR (policyname = 'MG_P_' || raw_role || '_SHARE_GROUP'
                          AND OLD.mg_roles && "MOLGENIS".current_user_roles())
                  )
            ) THEN
                allowed_share := true;
