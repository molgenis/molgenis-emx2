...
public class TestTruncate {
  ...
  public void testTruncate() {
    ...
    Table table2 = schema.create(table("Table2").setInheritNames("Table1").add(column("col1")));
    table1.insert(row("name", "a"));
    ...
    Table table3 = schema.create(table("Table3").setInheritNames("Table2").add(column("col2")));
    table1.insert(row("name", "a"));
    table2.insert(row("name", "b", "col1", "checkb"));
