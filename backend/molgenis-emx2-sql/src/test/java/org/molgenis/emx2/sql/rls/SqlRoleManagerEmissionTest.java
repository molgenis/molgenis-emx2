...
class SqlRoleManagerEmissionTest {
  ...
  void setup() {
    ...
    jooq.execute(
        "DELETE FROM \"" + MOLGENIS_SCHEMA + "\".role_metadata WHERE role_name LIKE 'emission_%'");
    dropTestPgRole();
  ...
  void roundTrip() {
    ...
    Permission resolved = retrieved.resolveFor(TEST_SCHEMA, TEST_TABLE);
