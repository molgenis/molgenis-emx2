...
public class TestRefBack {
  ...
  public void execute(ColumnType refArrayOrMref) {
    ...
    assertTrue(query.retrieveJSON().contains("{\"Parts_agg\" : {\"count\" : 5}}"));

    query = products.agg(s("count"));
    assertTrue(query.retrieveJSON().contains("{\"Products_agg\" : {\"count\" : 2}}"));

    query = products.select(s("parts_agg", s("count"))).where(f("productname", EQUALS, "bigphone"));

    assertTrue(
        query.retrieveJSON().contains("{\"Products\" : [{\"parts_agg\" : {\"count\" : 3}}]}"));

    query = parts.select(s("products_agg", s("count"))).where(f("partname", EQUALS, "battery"));

    assertTrue(
        query.retrieveJSON().contains("{\"Parts\" : [{\"products_agg\" : {\"count\" : 2}}]}"));
  ...
  public void testRefBack() {
    ...
    assertTrue(query.retrieveJSON().contains("{\"User_agg\" : {\"count\" : 2}}"));
    ...
    assertTrue(query.retrieveJSON().contains("{\"User_agg\" : {\"count\" : 1}}"));
