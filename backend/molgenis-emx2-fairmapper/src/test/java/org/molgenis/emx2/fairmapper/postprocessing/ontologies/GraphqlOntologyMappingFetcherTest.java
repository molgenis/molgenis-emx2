...
class GraphqlOntologyMappingFetcherTest {
  ...
  void shouldResolveOntologies() {
    GraphqlClient staticClient =
        getStaticClient(
            ...
            """);
    OntologyMappingFetcher mapper = new GraphqlOntologyMappingFetcher(staticClient);
  ...
  void shouldParseToPascalCase() {
    GraphqlClient staticClient =
        getStaticClient(
            ...
            "CountryNames");
    OntologyMappingFetcher mapper = new GraphqlOntologyMappingFetcher(staticClient);
    Map<String, String> mapping = mapper.getMapping("schema", "Country names");
    assertEquals(
        Map.of("http://publications.europa.eu/resource/authority/country/AFG", "Afghanistan"),
        mapping);
  }
  ...
  void shouldHandleNull() {
    ...
    OntologyMappingFetcher mapper = new GraphqlOntologyMappingFetcher(staticClient);
    MolgenisException exception =
        assertThrows(MolgenisException.class, () -> mapper.getMapping("schema", "Country names"));
    assertEquals(
        "Unable to convert json node to a list of OntologyMappings: null", exception.getMessage());
  }
  ...
  GraphqlClient getStaticClient(String expectedResponse, String tableName) {
    GraphqlClient graphqlClient = spy(new GraphqlClient("http://example.org", "some-token"));
    try {
      doReturn(MAPPER.readTree(expectedResponse))
          .when(graphqlClient)
