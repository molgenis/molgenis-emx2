...
public class SparqlSelectRdfTransformer implements RdfTransformer {
  ...
  private void addTableDataToStore(
      String table, SailRepository repository, StreamingTableStore tableStore) {
    SailRepositoryConnection conn = repository.getConnection();
    SelectQuery query = queryGenerator.generate(schema.getTableMetadata(table));
    TupleQuery prepared = conn.prepareTupleQuery(QueryLanguage.SPARQL, query.getQueryString());
