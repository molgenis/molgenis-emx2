...
/**
 * Represents a per-table permission grant for a role. Used for both schema-local and global roles.
 *
 * <p>A {@code null} table (or {@code "*"}) represents a schema-wide default that applies to all
 * RLS-enabled tables. Equality is based on (schema, table) so that permissions for the same table
 * can be merged.
 */
public class Permission {
  ...
  public Permission setSchema(String schema) {
    this.schema = schema;
    return this;
  }
  ...
  /** Returns true when all permission fields are null, indicating a revocation intent. */
  ...
  /** Returns true if any dimension uses ROW-level filtering. */
  ...
  /** Returns true when this is a schema-wide default (no specific table). */
