 ...
 *
 * <p>Exercises the full lifecycle: changePermissions → effectivePermissions → data access → UPDATE
 * scope enforcement → dropPermissions.
 *
 * <p>Note: placed in molgenis-emx2-webapi (not molgenis-emx2-nonparallel-tests as originally
 * specified) because the HTTP test infrastructure (ApiTestBase, RestAssured, MolgenisWebservice)
 * lives here.
 */
@Tag("slow")
class GraphqlPermissionsIT extends ApiTestBase {
  ...
  void fullScenario() throws Exception {
    ...
    // Step 3: single changePermissions mutation — creates role, grants SELECT=ALL + UPDATE=OWN,
    // enables RLS, adds non-admin user as member
    String changeMutation =
        "{\"query\":\"mutation{changePermissions("
            + "roles:[{name:\\\""
            + CLINICIAN_ROLE
            + "\\\"}],"
            + "permissions:[{role:\\\""
    ...
    assertEquals(
        "SUCCESS",
        changeResult.at("/data/changePermissions/status").asText(),
        "changePermissions must succeed: " + changeBody);

    // Step 4: non-admin queries _session.effectivePermissions — must show SELECT=ALL, UPDATE=OWN.
    // The effectivePermissions field is available on the database-level /api/graphql endpoint
    // (forDatabase wires sessionQueryField) as well as the schema-level endpoint.
    // We query /api/graphql because the non-admin may not have schema USAGE yet.
    ...
    String sessionBody =
        given()
            ...
            .body(
                "{\"query\":\"{ _session { effectivePermissions { schema table select update } } }\"}")
            .post(API_GQL)
            .then()
            .extract()
            .asString();

    JsonNode sessionPerms = MAPPER.readTree(sessionBody).at("/data/_session/effectivePermissions");
    assertFalse(
        sessionPerms.isMissingNode(),
        "effectivePermissions must be present in response: " + sessionBody);
    assertTrue(sessionPerms.isArray(), "effectivePermissions must be array");
    ...
    // The mutation returns successfully but with 0 rows affected, or may error.
    // Either way, the value must not change to "hacked".
    database.becomeAdmin();
    String verifyBody =
        given()
            .sessionId(adminSession)
    ...
    String dropBody =
        given()
            .sessionId(adminSession)
            .body(
                "{\"query\":\"mutation{dropPermissions(roles:[\\\""
                    + CLINICIAN_ROLE
                    + "\\\"]){status message}}\"}")
            .post(API_GQL)
            .then()
    ...
    assertEquals(
        "SUCCESS",
        dropResult.at("/data/dropPermissions/status").asText(),
        "dropPermissions must succeed: " + dropBody);

    // After drop: non-admin effectivePermissions must be empty for this schema+table.
    String postDropSessionBody =
        given()
            .sessionId(nonAdminSession)
            .body("{\"query\":\"{ _session { effectivePermissions { schema table } } }\"}")
            .post(API_GQL)
            .then()
            .extract()
            .asString();

    JsonNode postDropPerms =
        MAPPER.readTree(postDropSessionBody).at("/data/_session/effectivePermissions");
    ...
    assertFalse(
        permStillPresent,
        "After dropPermissions, effectivePermissions must not include the schema+table entry: "
            + postDropSessionBody);
  }
