...
public class AppsProxyService {
  ...
  static final String PATH = formatPath("/plugin");
  ...
  public static void create(Database database) {
    ...
    settingsSchema
        .create(new TableMetadata("Apps").add(new Column("path").setPkey()))
        .add(new Column(SOURCE));
    schema.migrate(settingsSchema);
    // load some defaults
    schema
        .getTable("Apps")
        .save(
  ...
  private static Object handleRequest(Request req, Response res) throws IOException {
    ...
    for (Row r : database.getSchema(SYSTEM).getTable("Apps").retrieveRows()) {
