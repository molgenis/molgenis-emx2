...
public class TestGraphqSchemaFields {
  ...
  public void testTableAlterDropOperations() throws IOException {
    ...
    // add table
    execute(
        "mutation{change(tables:[{name:\"table1\",descriptions:[{locale:\"en\", value: \"desc1\"}],columns:[{name:\"col1\", key:1, descriptions:[{locale:\"en\", value:\"desc11\"}]}]}]){message}}");
    ...
    assertEquals("Col1", node.at("/_schema/tables/5/columns/0/labels/0/value").asText());
    ...
    // default label should change when name changes

    // update with label not same as default
    execute(
        "mutation{change(tables:[{name:\"table1\",labels:[{locale:\"en\", value: \"table one\"}],descriptions:[{locale:\"en\", value: \"desc1\"}],columns:[{name:\"col1\", key:1, labels:[{locale:\"en\", value:\"column one\"}], descriptions:[{locale:\"en\", value:\"desc11\"}]}]}]){message}}");
    node =
        execute(
            "{_schema{tables{name,labels{locale,value},descriptions{locale,value},columns{name,key,labels{locale,value},descriptions{locale,value}}}}}");

    assertEquals("en", node.at("/_schema/tables/5/labels/0/locale").asText());
    assertEquals("table one", node.at("/_schema/tables/5/labels/0/value").asText());

    assertEquals("en", node.at("/_schema/tables/5/columns/0/labels/0/locale").asText());
    assertEquals("column one", node.at("/_schema/tables/5/columns/0/labels/0/value").asText());
