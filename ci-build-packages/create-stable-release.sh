...
curl -v -u $NEXUS_USER:$NEXUS_PASS --upload-file molgenis-emx2-$VERSION-$RELEASE.noarch.rpm     https://registry.molgenis.org/repository/molgenis-emx2-yum/
echo "Upload DEB to apt noble  registry"
curl -v -u $NEXUS_USER:$NEXUS_PASS -H Content-Type: multipart/form-data --data-binary @./molgenis-emx2_$VERSION-${RELEASE}_all.deb https://registry.molgenis.org/repository/molgenis-emx2-noble/
echo "Upload RPM to apt jammy registry"
curl -v -u $NEXUS_USER:$NEXUS_PASS -H Content-Type: multipart/form-data --data-binary @./molgenis-emx2_$VERSION-${RELEASE}_all.deb https://registry.molgenis.org/repository/molgenis-emx2-jammy/
